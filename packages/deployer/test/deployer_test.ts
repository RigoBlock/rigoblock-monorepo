...
describe('#Deployer', () => {
  ...
  describe('#deployAsync', () => {
    it('should deploy the Exchange contract without updating the Exchange artifact', async () => {
      const exchangeConstructorArgs = [
        constants.zrxTokenAddress,
        constants.tokenTransferProxyAddress
      ]
      const exchangeContractInstance = await deployer.deployAsync(
        'Exchange',
        exchangeConstructorArgs
      )
      const opts = {
        encoding: 'utf8'
      }
  ...
  describe('#deployAndSaveAsync', () => {
    it('should save the correct contract address and constructor arguments to the Exchange artifact', async () => {
      ...
      const exchangeContractInstance = await deployer.deployAndSaveAsync(
        'Exchange',
        exchangeConstructorArgs
      )
      const opts = {
      ...
      expect(constructor_args).to.be.equal(
        exchangeContractData.constructor_args
      )
    })
  })
})
