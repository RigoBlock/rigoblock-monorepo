...
describe('Compiler utils', () => {
  describe('#getNormalizedErrorMessage', () => {
    it('normalizes the error message', () => {
      ...
      expect(normalizedErrMsg).to.be.equal(
        'Token.sol:6:46: Warning: Unused local variable'
      )
    })
  })
  ...
  describe('#parseSolidityVersionRange', () => {
    it('correctly parses the version range', () => {
      expect(parseSolidityVersionRange('pragma solidity ^0.0.1;')).to.be.equal(
        '^0.0.1'
      )
      expect(parseSolidityVersionRange('\npragma solidity 0.0.1;')).to.be.equal(
        '0.0.1'
      )
      expect(parseSolidityVersionRange('pragma solidity <=1.0.1;')).to.be.equal(
        '<=1.0.1'
      )
      expect(
        parseSolidityVersionRange('pragma solidity   ~1.0.1;')
      ).to.be.equal('~1.0.1')
    })
  ...
  describe('#parseDependencies', () => {
    it('correctly parses Exchange dependencies', async () => {
      ...
      _.each(expectedDependencies, expectedDepdency => {
        const foundDependency = _.find(dependencies, dependency =>
          _.endsWith(dependency, expectedDepdency)
        )
        expect(
          foundDependency,
          `${expectedDepdency} not found`
        ).to.not.be.undefined()
      })
    })
