...
export class Compiler {
  ...
  private async _compileContractAsync(contractName: string): Promise<void> {
    ...
    const currentArtifactIfExists = await getContractArtifactIfExistsAsync(
      this._artifactsDir,
      contractName
    )
    const sourceTreeHashHex = `0x${this._getSourceTreeHash(
      contractSource.path
    ).toString('hex')}`
    ...
    if (_.isUndefined(currentArtifactIfExists)) {
      ...
      shouldCompile =
        ...
        currentArtifact.networks[this._networkId].source_tree_hash !==
          sourceTreeHashHex
    }
    if (!shouldCompile) {
      return
    ...
    if (!_.isUndefined(compiled.errors)) {
      ...
      if (!_.isEmpty(errors)) {
        errors.forEach(error => {
          const normalizedErrMsg = getNormalizedErrMsg(
  ...
  private _getSourceTreeHash(importPath: string): Buffer {
    ...
    if (dependencies.length === 0) {
      ...
      const dependencySourceTreeHashes = _.map(
        dependencies,
        (dependency: string) => this._getSourceTreeHash(dependency)
      )
