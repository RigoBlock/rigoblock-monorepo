...
export class Deployer {
  ...
  constructor(opts: DeployerOptions) {
    ...
    if (_.isUndefined((opts as ProviderDeployerOptions).provider)) {
      ...
      if (_.isUndefined(jsonrpcUrl)) {
        throw new Error(
          `Deployer options don't contain provider nor jsonrpcUrl. Please pass one of them`
        )
      }
  ...
  public async deployAsync(
    ...
    const contractArtifactIfExists: ContractArtifact = this._loadContractArtifactIfExists(
      contractName
    )
    ...
    const constructorAbi = _.find(abi, {
      type: AbiType.Constructor
    }) as ConstructorAbi
    const constructorArgs = _.isUndefined(constructorAbi)
      ? []
      : constructorAbi.inputs
    if (constructorArgs.length !== args.length) {
      const constructorSignature = `constructor(${_.map(
        constructorArgs,
        arg => `${arg.type} ${arg.name}`
      ).join(', ')})`
      throw new Error(
        `${contractName} expects ${
          constructorArgs.length
        } constructor params: ${constructorSignature}. Got ${args.length}`
      )
    }
    const web3ContractInstance = await this._deployFromAbiAsync(
      abi,
      args,
      txData
    )
   ...
   * @return Deployed contract instance.
   */
  public async deployAndSaveAsync(
    contractName: string,
    args: any[] = []
  ): Promise<Web3.ContractInstance> {
    const contractInstance = await this.deployAsync(contractName, args)
    await this._saveContractDataToArtifactAsync(
      contractName,
   ...
   */
  private async _deployFromAbiAsync(
    abi: ContractAbi,
    args: any[],
    txData: TxData
  ): Promise<any> {
    ...
    > = this.web3Wrapper.getContractFromAbi(abi)
  ...
  private async _saveContractDataToArtifactAsync(
    ...
    const abi = contractNetworkDataIfExists.abi
    const encodedConstructorArgs = encoder.encodeConstructorArgsFromAbi(
      args,
      abi
    )
   ...
   */
  private _loadContractArtifactIfExists(
    contractName: string
  ): ContractArtifact {
    const artifactPath = `${this._artifactsDir}/${contractName}.json`
    try {
      ...
      throw new Error(
        `Artifact not found for contract: ${contractName} at ${artifactPath}`
      )
    }
  }
  /**
   ...
   */
  private _getContractNetworkDataFromArtifactIfExists(
    contractArtifact: ContractArtifact
  ): ContractNetworkData {
    const contractNetworkDataIfExists =
      contractArtifact.networks[this._networkId]
    if (_.isUndefined(contractNetworkDataIfExists)) {
      throw new Error(
        `Data not found in artifact for contract: ${
          contractArtifact.contract_name
        }`
      )
    }
  ...
  private async _getAllowableGasEstimateAsync(data: string): Promise<number> {
    ...
    let gas: number
    try {
      const gasEstimate: number = await this.web3Wrapper.estimateGasAsync({
        data
      })
