...
export class Contract implements Web3.ContractInstance {
  ...
  private _populateFunctions(): void {
    const functionsAbi = _.filter(
      this.abi,
      abiPart => abiPart.type === AbiType.Function
    ) as FunctionAbi[]
    _.forEach(functionsAbi, (functionAbi: MethodAbi) => {
      if (functionAbi.constant) {
        ...
        const cbStyleCallFunction = this._contract[functionAbi.name].call
        const cbStyleEstimateGasFunction = this._contract[functionAbi.name]
          .estimateGas
        this[functionAbi.name] = this._promisifyWithDefaultParams(
          cbStyleFunction
        )
        this[functionAbi.name].estimateGasAsync = promisify(
          cbStyleEstimateGasFunction
        )
        this[
          functionAbi.name
        ].sendTransactionAsync = this._promisifyWithDefaultParams(
          cbStyleFunction
        )
        this[functionAbi.name].call = promisify(
          cbStyleCallFunction,
  ...
  private _populateEvents(): void {
    const eventsAbi = _.filter(
      this.abi,
      abiPart => abiPart.type === AbiType.Event
    ) as EventAbi[]
    _.forEach(eventsAbi, (eventAbi: EventAbi) => {
      this[eventAbi.name] = this._contract[eventAbi.name]
    })
  }
  private _promisifyWithDefaultParams(
    fn: (...args: any[]) => void
