...
export class ASelfCustodyContract extends BaseContract {
    /**
     * @ignore
     */
public static deployedBytecode: string | undefined;
public static contractName = 'ASelfCustody';
...
public static async deployFrom0xArtifactAsync(
        ...
        return ASelfCustodyContract.deployAsync(bytecode, abi, provider, txDefaults, logDecodeDependenciesAbiOnly, );
    }
    ...
    public static async deployWithLibrariesFrom0xArtifactAsync(
        ...
        const libraryAddresses = await ASelfCustodyContract._deployLibrariesAsync(
            artifact,
            libraryArtifacts,
    ...
    public static async deployAsync(
        ...
        logUtils.log(`ASelfCustody successfully deployed at ${txReceipt.contractAddress}`);
        const contractInstance = new ASelfCustodyContract(txReceipt.contractAddress as string, provider, txDefaults, logDecodeDependencies);
        ...
        for (const link of Object.values(links)) {
            for (const libraryName of Object.keys(link)) {
                if (!libraryAddresses[libraryName]) {
                    ...
                    await ASelfCustodyContract._deployLibrariesAsync(
                        libraryArtifact,
                        libraryArtifacts,
                        web3Wrapper,
                        txDefaults,
    ...
    public getFunctionSignature(methodName: string): string {
        ...
        const methodAbi = ASelfCustodyContract.ABI()[index] as MethodAbi; // tslint:disable-line:no-unnecessary-type-assertion
...
> {
        const self = this as any as ASelfCustodyContract;
            assert.isString('selfCustodyAccount', selfCustodyAccount);
            assert.isString('token', token);
            assert.isBigNumber('amount', amount);
        const functionSignature = 'transferToSelfCustody(address,address,uint256)';
...
> {
    ...
    constructor(
        ...
        txDefaults?: Partial<TxData>,
        logDecodeDependencies?: { [contractName: string]: ContractAbi },
        deployedBytecode: string | undefined = ASelfCustodyContract.deployedBytecode,
    ) {
        super('ASelfCustody', ASelfCustodyContract.ABI(), address, supportedProvider, txDefaults, logDecodeDependencies, deployedBytecode);
        classUtils.bindAll(this, ['_abiEncoderByFunctionSignature', 'address', '_web3Wrapper']);
ASelfCustodyContract.ABI().forEach((item, index) => {
