...
contract SelectorProviderMock is SelectorProvider {
    function getSelector(bytes4 genericSelector) public pure returns (bytes4) {
        if (genericSelector == getAmountToGiveSelector) {
            return bytes4(keccak256("getAmountToGive((uint256))"));
        } else if (genericSelector == staticExchangeChecksSelector) {
            return bytes4(keccak256("staticExchangeChecks((uint256))"));
        } else if (genericSelector == performBuyOrderSelector) {
            return bytes4(keccak256("performBuyOrder((uint256),uint256)"));
        } else if (genericSelector == performSellOrderSelector) {
            return bytes4(keccak256("performSellOrder((uint256),uint256)"));
        } else {
            return bytes4(0x0);
        }
    }
...
contract HandlerMock is ExchangeHandler, AllowanceSetter, SelectorProviderMock {
    ...
    {
        amountSpentOnOrder = amountToGiveForOrder;
        token.transfer(address(0x1), amountToGiveForOrder);
        amountReceivedFromOrder = amountSpentOnOrder / priceDivider;
